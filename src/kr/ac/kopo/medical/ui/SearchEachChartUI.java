...
public class SearchEachChartUI extends ChartBaseUI {
	...
	public void execute() throws Exception {
		...
		// 화면 출력
		// 문자열 포맷
		String leftAlignFormat = "| %-10s | %-20s | %-20s | %-20s | %-20s | %-20s | %-30s | %-30s |%n";
		...
		if(list.isEmpty()) {
			...
			for (MedicalChartVO chart : list) {
				System.out.format(leftAlignFormat, chart.getChartId(), chart.getAnimalId(), chart.getAnimalName(), chart.getVetId(), chart.getVetName(), chart.getTreatDate(), chart.getDiagnosis(), chart.getTreatment());
			}			
		}
		System.out.println("*------------*----------------------*----------------------*----------------------*----------------------*----------------------*--------------------------------*--------------------------------*");
